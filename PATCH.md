...
- Changes:
  - Moved Diffs, Git Graph, Terminal from the overflow menu into always-visible header buttons, ordered [+, Diffs, Git Graph, Terminal, ⋮]; removed the menu items, their emits, and the now-unused hasCwd prop.
- Reverted:
  - The Diffs/Git Graph/Terminal header buttons were moved back into the overflow menu (with their emits and hasCwd prop restored).
