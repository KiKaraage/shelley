...
Base: 9c96638
Files: ui/src/styles.css
Changes: Changes to `.status-bar` and `.message-input-container` to make them visually seamless:
...
Changes: Added a tag picker dropdown to the chat header, positioned in `.header-left` after `.header-title`.
  - **HeaderTagPicker.vue**: `pi pi-tag` icon button using `btn-icon` (same sizing as Diffs/GitGraph/Terminal buttons), `text severity="secondary"`. Opens a dropdown mirroring the group-by-menu pattern (absolute-positioned `div`, outside-click dismissal via `mousedown` listener). Input at top auto-focused on open, filters the fixed suggested tag list by substring. Tags: `explore`, `plan`, `ongoing`, `human-verify`, `revise`, `done`. Already-applied tags show a `✓` checkmark; click toggles apply/remove via `api.updateConversationTags`. Typed text not matching any suggested tag shows a `+ Create "#text"` row; Enter applies it. No delete buttons — removal lives in the drawer. Escape closes the dropdown.
...
- The copy/fork/details overlay now appears per-block instead of once for the whole message.
- Only thinking and text blocks get their own action bar; tool blocks, [Operation cancelled] blocks, and other non-text/non-thinking blocks get nothing.
- Each block hover tracks its index; the action bar copies only that block's text.
- Block wrappers are full-width with padding-top/margin-top so the hover zone extends upward to cover the action bar.
