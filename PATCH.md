...
- Changes:
  ...
  - **ChatInterface.vue**: `TagPicker` wraps a `<button class="app-bar-title header-title header-title-btn">` session name via its `#trigger` slot — clicking the title opens the tag picker instead of using a separate icon button. Title is a `<button>` (not `<h1>`) so it has proper semantics and gets `:hover` background via `.header-title-btn`. The button also renders the first tag inline (`#tag`) with a trailing `…` if more tags exist, giving immediate visual feedback without opening the picker. `firstTag` and `hasMoreTags` computed props parse the conversation's JSON tags array. HeaderTagPicker import removed.
  - **styles.css**: `.tag-picker-wrapper`, `.tag-picker-menu` (min-width 10rem), `.tag-picker-input`, `.tag-picker-item`, `.tag-picker-check`, `.tag-picker-hash`, `.tag-picker-create`, `.tag-picker-empty`: follows the same design tokens as `.group-by-menu`. Removed `overflow: hidden` from `.header-left` so the dropdown floats outside the header area. `.header-title-btn` has `min-width: 0` so long session names truncate with ellipsis instead of overflowing into the header action buttons.
- Watchouts:
...
- Changes:
  ...
  - Fix task not gaining its thread link after "Start thread": `handleFirstMessage` now calls `loadTasks()` after the conversation is created, so the task immediately moves to handled and shows its thread badge instead of staying idle until the next full page load.
...
## P043
Show HomePage by default on root path instead of auto-selecting most recent conversation
- Status: active
- Base: 10cd8c9
